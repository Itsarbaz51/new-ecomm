<x-app-layout title="E.COM.SELLER" bodyClass="gradient-bg">
    <section class="swiper-container js-swiper-slider w-full h-[70vh] md:h-[80vh] lg:h-[90vh] relative" data-settings='{
        ...
        <div class="swiper-wrapper w-full h-full">
            ...
            <div class="swiper-slide w-full h-full relative">
                <img src="{{ asset('storage/uploads/slides/' . $slide->image) }}" alt="{{ $slide->title }}" class="w-full h-full object-cover" loading="lazy" />
    ...
    <section class="products-tabs container">
        ...
        <div class="tabs overflow-auto">
            ...
            <a href="{{ route('home.index', ['categories' => $category->id]) }}" class="tab {{ $activeCategory == $category->id ? 'active' : '' }}">
                        ...
                        <div class="card h-100 w-72 lg:w-fit shadow-sm product-card product-hover">
                                    ...
                                    <div class="swiper-wrapper">
                                        <div class="swiper-slide flex justify-center items-center">
                                            <a href="{{ route('shop.product.details', ['product_slug' => $product->slug]) }}">
                                                <img src="{{ asset('storage/uploads/products/thumbnails/' . $product->image) }}" class="object-contain" alt="{{ $product->name }}">
                                            </a>
                                        </div>
                                        @foreach (explode(',', $product->images) as $image)
                                        <div class="swiper-slide">
                                            <a href="{{ route('shop.product.details', ['product_slug' => $product->slug]) }}">
                                                <img src="{{ asset('storage/uploads/products/gallery/' . trim($image)) }}" class="card-img-top object-contain product-image" alt="{{ $product->name }}">
                                            </a>
                                        </div>
                                        @endforeach
                                    </div>
                            ...
                            <div class="card-body text-center">
                                ...
                                <h6 class="card-title mb-2">
                                    <a href="{{ route('shop.product.details', ['product_slug' => $product->slug]) }}" class="text-dark text-decoration-none">{{ $product->name }}</a>
                                </h6>
                                ...
                                <div class="d-flex justify-content-center align-items-center mb-3">
                                    <div class="product-single__rating">
                                        <div class="reviews-group d-flex">
                                            ...
                                            {{-- Filled stars --}}
                                            @for ($i = 1; $i <= $roundedRating; $i++) <svg class="review-star" viewBox="0 0 9 9" xmlns="http://www.w3.org/2000/svg" fill="#FFD700">
                                                <use href="#icon_star" />
                                                </svg>
                                                @endfor

                                                {{-- Empty stars --}}
                                                @for ($i = $roundedRating + 1; $i <= 5; $i++) <svg class="star-rating__star-icon" width="12" height="12" fill="#ccc" viewBox="0 0 12 12" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="py-4 px-4 md:px-6 lg:px-8">
            <div class="grid grid-cols-1 sm:grid-cols-2 xxl:grid-cols-4 gap-6">
                ...
                <div class="bg-white relative rounded-2xl shadow flex flex-col justify-center hover:shadow-md transition-all overflow-hidden">
                    <img loading="lazy" src="{{ asset('storage/uploads/categories/' . $categorie->image) }}" alt="{{ $categorie->name }}" class="object-contain w-full max-h-[500px] rounded-lg" />
                ...
                <div class="testimonial-info">
                    ...
                    <div class="testimonial-reviews flex items-center justify-center">
                        @for ($i = 1; $i <= $review->rating; $i++)
                            ...
                            @endfor
                            @for ($i = $review->rating + 1; $i <= 5; $i++) <svg class="w-5 h-5 text-gray-600" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.962h4.163c.969 0 1.371 1.24.588 1.81l-3.37 2.448 1.287 3.962c.3.921-.755 1.688-1.538 1.118L10 13.347l-3.37 2.448c-.783.57-1.838-.197-1.538-1.118l1.287-3.962-3.37-2.448c-.783-.57-.38-1.81.588-1.81h4.163l1.286-3.962z" />
                                </svg>
                                @endfor
                    </div>
